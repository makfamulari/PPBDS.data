...
shovel_final <- shovels %>%
 ggplot(aes(x = shovel_size, y = prop_red_results)) +
 geom_point() +
 labs(x = "Shovel size",
...
We will now modify the object shovels by using a mutate to create the column `use_shovel_results`. `use_shovel_results` should use map `shovel_size` by using our function `use_shovel` with x = urn, size = .x, and reps = 1000. If this sounds confusing, make sure you've looked at "Functions are your friends" carefully.
...
Now that we have finished modifying our object `shovels`, let's construct our graphing using ggplot!

Start by calling `shovels` and then using ggplot with the `shovel_size` as the x-axis and `prop_red_results` as the y-axis. Add geom_point. 
...
Because we want to be more precise than the previous exercise, let's see if we can look at shovel sizes from 1 to 100, instead of for 30, 60, and 120. Instead of our first line being `shovels` lets start by calling tibble to redefine shovel_size for all values between 1 and 100.
...
Now that we are using our functions to draw for all shovel sizes between 1 and 100, we will need to use mutate to create a column that takes the standard deviation of the proportion red found from `prop_red_results`. After copying the above work, add the column `prop_red_sd` using `map_dbl` with prop_red_results. After the ~ sign, you will want to pull using (., prop_red) to isolate the proportion red. Finally, you will take the `sd()` of that pulled value.

This new work should be saved as `shovels_100` instead of the previous `shovels`.
...
As we have seen, we now have the standard deviation of proportions red for all shovel sizes between 1 and 100. We now want to use `ggplot` to visualize this difference in a `geom_point`. Assign `shovel_size` to the x-axis and `prop_red_sd` to the y-axis. 
...
Nice work! Let's finish this visualization by using `labs()` to label the x-axis as "Shovel size", the y-axis as "Standard deviation of the proportion red", and the title as "The impact of shovel size on standard deviation".
...
shovels_100 %>%
 ...
 labs(x = "Shovel size",
      ...
      title = "The impact of shovel size on standard deviation")
```
